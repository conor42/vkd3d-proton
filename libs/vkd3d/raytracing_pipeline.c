        ...
        const struct vkd3d_shader_library_entry_point *entry)
{
    ...
    if (!rs)
    {
        if (data->high_priority_local_root_signature)
            rs = unsafe_impl_from_ID3D12RootSignature(data->high_priority_local_root_signature);
        else if (data->low_priority_local_root_signature)
            rs = unsafe_impl_from_ID3D12RootSignature(data->low_priority_local_root_signature);
        else
            rs = NULL;
    }
